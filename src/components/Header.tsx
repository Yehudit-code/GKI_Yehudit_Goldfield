...
export default function Header() {
  ...
  return (
    <header className="bg-white shadow-md fixed top-0 w-full z-50 h-[13vh]">
        ...
        <nav className="hidden md:flex gap-6">
          {categories.map((cat) => (
            <Link
              key={cat.name}
              href={cat.href}
              className={`font-medium ${cat.name === "Home" ? "text-yellow-500" : "text-gray-700 hover:text-yellow-500"
                }`}
            >
              {cat.name}
            </Link>
